...
async def create_public_order(order_data: PublicOrderCreate, db: AsyncSession = Depends(get_db_session)):
    ...
    if order_data.type != OrderType.DINE_IN:
        raise HTTPException(
            ...
            detail="Only dine-in orders are allowed without authentication. Please register/login for delivery or takeaway orders.",  # noqa: E501
        )
    ...
    if order_data.tableId:
        result = await db.execute(
            select(Table).where(
                and_(
                    Table.id == order_data.tableId,
                    Table.restaurantId == order_data.restaurantId,
                )
            )
        )
    ...
    for item in order_data.items:
        ...
        if dish.quantity < item.quantity:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Not enough quantity for dish '{dish.name}'. Available: {dish.quantity}, Requested: {item.quantity}",  # noqa: E501
            )
    ...
    if total_amount > MAX_PUBLIC_ORDER_AMOUNT:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Order amount ({total_amount}) exceeds maximum allowed for public orders ({MAX_PUBLIC_ORDER_AMOUNT}). Please register/login for larger orders.",  # noqa: E501
        )
...
async def create_order(
    ...
    result = await db.execute(select(User).where(User.id == current_user.id).options(selectinload(User.address)))
    ...
    if order_data.type == OrderType.DELIVERY:
        if not order_data.deliveryAddressId and not user_profile.address:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Delivery address is required. Please add an address to your profile or specify a delivery address.",  # noqa: E501
            )
...
async def create_delivery_order(
    ...
    if order_data.useStoredAddress:
        ...
        if not user_profile.address:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No stored address found. Please add an address to your profile or provide a custom delivery address.",  # noqa: E501
            )
