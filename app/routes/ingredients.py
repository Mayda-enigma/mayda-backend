...
async def create_ingredient(
    ...
    stmt = select(Ingredient).where(Ingredient.name == ingredient_data.name, Ingredient.isActive == True)
...
async def get_ingredients(
    ...
    has_allergens: bool | None = Query(None),
    ...
    try:
        stmt = select(Ingredient).where(and_(*conditions)).offset(skip).limit(limit).order_by(Ingredient.name.asc())
...
async def get_ingredient_categories(
    ...
    try:
        ...
        result = [IngredientCategoryResponse.model_validate(data) for data in category_data.values()]
...
async def get_ingredient(
    ...
    # Count dishes using this ingredient
    count_stmt = select(func.count(Dish.id)).where(Dish.ingredients.any(DishIngredient.ingredientId == ingredient_id))
    dish_count = (await db.execute(count_stmt)).scalar()
...
async def delete_ingredient(
    ...
    if dish_count > 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot delete ingredient '{ingredient.name}' as it is used in {dish_count} dish(es). Remove from dishes first.",  # noqa: E501
        )
...
async def get_dish_ingredients(
    ...
    stmt = (
        ...
        .options(
            ...
            selectinload(Dish.category).selectinload(MenuCategory.menu).selectinload(Menu.restaurant),
        )
