...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE \"ReservationStatus\" AS ENUM ('PENDING', 'CONFIRMED', 'CANCELLED', 'COMPLETED', 'NO_SHOW')")
    op.execute("CREATE TYPE \"OrderType\" AS ENUM ('DINE_IN', 'TAKEAWAY', 'DELIVERY')")
    op.execute("CREATE TYPE \"OrderStatus\" AS ENUM ('PENDING', 'CONFIRMED', 'PREPARING', 'READY', 'OUT_FOR_DELIVERY', 'COMPLETED', 'CANCELLED')")
    ...
    op.create_table("restaurants",
        ...
        sa.Column("gallery", postgresql.ARRAY(sa.Text()), nullable=False, server_default=sa.text("'{}'")),
    ...
    op.create_table("users",
        ...
        sa.Column("role", postgresql.ENUM("CLIENT", "WAITER", "CHEF", "MANAGER", "ADMIN", name="UserRole", create_type=False), nullable=False, server_default=sa.text("'CLIENT'")),
        sa.Column("isActive", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column("createdAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updatedAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
    ...
    op.create_foreign_key("users_restaurantId_fkey", "users", "restaurants", ["restaurantId"], ["id"], ondelete="SET NULL")
    ...
    op.create_foreign_key("refresh_tokens_userId_fkey", "refresh_tokens", "users", ["userId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("push_tokens_userId_fkey", "push_tokens", "users", ["userId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("addresses_userId_fkey", "addresses", "users", ["userId"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("addresses_restaurantId_fkey", "addresses", "restaurants", ["restaurantId"], ["id"], ondelete="CASCADE")
    ...
    op.create_table("tables",
        ...
        sa.Column("isActive", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column("status", postgresql.ENUM("AVAILABLE", "OCCUPIED", name="TableStatus", create_type=False), nullable=False, server_default=sa.text("'AVAILABLE'")),
    ...
    op.create_foreign_key("tables_restaurantId_fkey", "tables", "restaurants", ["restaurantId"], ["id"], ondelete="CASCADE")
    ...
    op.create_table("table_sessions",
        ...
        sa.Column("startedAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("endedAt", sa.DateTime(timezone=True), nullable=True),
        sa.Column("isActive", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_foreign_key("table_sessions_tableId_fkey", "table_sessions", "tables", ["tableId"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("table_sessions_waiterId_fkey", "table_sessions", "users", ["waiterId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("menus_restaurantId_fkey", "menus", "restaurants", ["restaurantId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("menu_categories_menuId_fkey", "menu_categories", "menus", ["menuId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("dishes_categoryId_fkey", "dishes", "menu_categories", ["categoryId"], ["id"], ondelete="CASCADE")
    ...
    op.create_table("reservations",
        ...
        sa.Column("status", postgresql.ENUM("PENDING", "CONFIRMED", "CANCELLED", "COMPLETED", "NO_SHOW", name="ReservationStatus", create_type=False), nullable=False, server_default=sa.text("'PENDING'")),
        sa.Column("createdAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updatedAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_foreign_key("reservations_userId_fkey", "reservations", "users", ["userId"], ["id"])
    op.create_foreign_key("reservations_tableId_fkey", "reservations", "tables", ["tableId"], ["id"], ondelete="SET NULL")
    op.create_foreign_key("reservations_restaurantId_fkey", "reservations", "restaurants", ["restaurantId"], ["id"])
    ...
    op.create_table("orders",
        ...
        sa.Column("restaurantId", sa.Integer(), nullable=False),
        sa.Column("tableId", sa.Integer(), nullable=True),
        sa.Column("type", postgresql.ENUM("DINE_IN", "TAKEAWAY", "DELIVERY", name="OrderType", create_type=False), nullable=False, server_default=sa.text("'DINE_IN'")),
        sa.Column("status", postgresql.ENUM("PENDING", "CONFIRMED", "PREPARING", "READY", "OUT_FOR_DELIVERY", "COMPLETED", "CANCELLED", name="OrderStatus", create_type=False), nullable=False, server_default=sa.text("'PENDING'")),
        ...
        sa.Column("paymentStatus", postgresql.ENUM("PENDING", "PAID", "FAILED", "REFUNDED", name="PaymentStatus", create_type=False), nullable=False, server_default=sa.text("'PENDING'")),
        sa.Column("paymentMethod", sa.Text(), nullable=True),
        sa.Column("notes", sa.Text(), nullable=True),
        sa.Column("orderTime", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
    ...
    op.create_foreign_key("orders_tableId_fkey", "orders", "tables", ["tableId"], ["id"], ondelete="SET NULL")
    op.create_foreign_key("orders_deliveryAddressId_fkey", "orders", "addresses", ["deliveryAddressId"], ["id"], ondelete="SET NULL")
    ...
    op.create_foreign_key("order_items_orderId_fkey", "order_items", "orders", ["orderId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("payments_orderId_fkey", "payments", "orders", ["orderId"], ["id"], ondelete="CASCADE")
    # Backlink FK order.paymentId → payments.id (circular, added after both tables exist)
    op.create_foreign_key("orders_paymentId_fkey", "orders", "payments", ["paymentId"], ["id"], ondelete="SET NULL")
    ...
    op.create_foreign_key("loyalty_cards_userId_fkey", "loyalty_cards", "users", ["userId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("loyalty_transactions_loyaltyCardId_fkey", "loyalty_transactions", "loyalty_cards", ["loyaltyCardId"], ["id"])
    op.create_foreign_key("loyalty_transactions_restaurantId_fkey", "loyalty_transactions", "restaurants", ["restaurantId"], ["id"])
    ...
    op.create_foreign_key("reviews_dishId_fkey", "reviews", "dishes", ["dishId"], ["id"], ondelete="SET NULL")
    ...
    op.create_table("promotions",
        ...
        sa.Column("description", sa.Text(), nullable=False),
        sa.Column("image", sa.Text(), nullable=True),
        sa.Column("type", postgresql.ENUM("DISCOUNT", "BOGO", "FREE_DELIVERY", "HAPPY_HOUR", "SEASONAL", name="PromotionType", create_type=False), nullable=False),
        sa.Column("discountType", postgresql.ENUM("PERCENTAGE", "FIXED_AMOUNT", name="DiscountType", create_type=False), nullable=False),
    ...
    op.create_foreign_key("promotions_restaurantId_fkey", "promotions", "restaurants", ["restaurantId"], ["id"])

    # 19. promotion_dishes (FK: promotionId → promotions.id, dishId → dishes.id)
    op.create_table("promotion_dishes",
        sa.Column("promotionId", sa.Integer(), nullable=False),
        sa.Column("dishId", sa.Integer(), nullable=False),
        sa.PrimaryKeyConstraint("promotionId", "dishId"),
    )
    op.create_foreign_key("promotion_dishes_promotionId_fkey", "promotion_dishes", "promotions", ["promotionId"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("promotion_dishes_dishId_fkey", "promotion_dishes", "dishes", ["dishId"], ["id"], ondelete="CASCADE")
    ...
    op.create_foreign_key("inventory_restaurantId_fkey", "inventory", "restaurants", ["restaurantId"], ["id"])
    ...
    op.create_foreign_key("ingredient_InventoryId_fkey", "ingredient", "inventory", ["InventoryId"], ["id"])
    ...
    op.create_table("otp_codes",
        ...
        sa.Column("purpose", postgresql.ENUM("STAFF_AUTH", "PAYMENT_CONFIRMATION", "PASSWORD_RESET", name="OtpPurpose", create_type=False), nullable=False),
        sa.Column("isUsed", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("expiresAt", sa.DateTime(timezone=True), nullable=False),
        sa.Column("createdAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_foreign_key("otp_codes_userId_fkey", "otp_codes", "users", ["userId"], ["id"], ondelete="CASCADE")

    # 24. platform_settings (no FKs)
    op.create_table("platform_settings",
        sa.Column("id", sa.Integer(), autoincrement=False, nullable=False, server_default=sa.text("1")),
        ...
        sa.Column("defaultOperatingHours", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("featureFlags", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("updatedAt", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.PrimaryKeyConstraint("id"),
    )
